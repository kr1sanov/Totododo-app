...
export function Archive() {
  ...
  const { archivedItems, restoreItem, deleteItem } = useArchive()

  const filteredItems = activeTab === "all" ? archivedItems : archivedItems.filter((item) => item.type === activeTab)
        ...
        <TabsContent value={activeTab} className="space-y-4">
          {filteredItems.length > 0 ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        Архивировано: {format(new Date(item.archivedAt!), "d MMM yyyy", { locale: ru })}
                      </div>
                    </div>
                    <DropdownMenu>
            ...
            <div className="text-center py-8 text-muted-foreground">
              В архиве нет {activeTab === "all" ? "элементов" : activeTab === "event" ? "событий" : "задач"}
            </div>
          )}
        </TabsContent>
      </Tabs>
